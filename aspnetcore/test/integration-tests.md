...
1. Herdar `WebApplicationFactory` de e substituir [ConfigureWebHost](/dotnet/api/microsoft.aspnetcore.mvc.testing.webapplicationfactory-1.configurewebhost). O [IWebHostBuilder](/dotnet/api/microsoft.aspnetcore.hosting.iwebhostbuilder) permite a configuração da coleção de serviços com [configuraservices](/dotnet/api/microsoft.aspnetcore.hosting.istartup.configureservices):
   ...
   A `InitializeDbForTests` propagação de banco de dados no [aplicativo de exemplo](https://github.com/dotnet/AspNetCore.Docs/tree/master/aspnetcore/test/integration-tests/samples) é executada pelo método. O método é descrito na seção [testes de integração exemplo: testar organização de aplicativos](#test-app-organization) .

   O contexto do banco de dados do SUT é `Startup.ConfigureServices` registrado em seu método. O retorno de chamada `builder.ConfigureServices` do aplicativo de teste é executado *depois* que o código do `Startup.ConfigureServices` aplicativo é executado. A ordem de execução é uma alteração significativa para o [host genérico](xref:fundamentals/host/generic-host) com o lançamento do ASP.NET Core 3,0. Para usar um banco de dados diferente para os testes do que o banco de dados do aplicativo, o contexto do banco `builder.ConfigureServices`de dados do aplicativo deve ser substituído em.

   O aplicativo de exemplo localiza o descritor de serviço para o contexto do banco de dados e usa o descritor para remover o registro do serviço. Em seguida, a fábrica adiciona um `ApplicationDbContext` novo que usa um banco de dados na memória para os testes.

   Para se conectar a um banco de dados diferente do banco de dados na memória `UseInMemoryDatabase` , altere a chamada para conectar o contexto a um banco de dados diferente. Para usar um banco de dados de teste SQL Server:
...
O `TestAuthHandler` é chamado para autenticar um usuário quando o esquema de autenticação é `Test` definido `AddAuthentication` como onde o `ConfigureTestServices`está registrado para:
...
| *IntegrationTests* | Contém os testes de integração para a página de índice `WebApplicationFactory` usando a classe personalizada. |
...
O contexto do banco de dados do SUT é `Startup.ConfigureServices` registrado em seu método. O retorno de chamada `builder.ConfigureServices` do aplicativo de teste é executado *depois* que o código do `Startup.ConfigureServices` aplicativo é executado. Para usar um banco de dados diferente para os testes, o contexto do banco de dados do `builder.ConfigureServices`aplicativo deve ser substituído em. Para obter mais informações, consulte a seção [Personalizar WebApplicationFactory](#customize-webapplicationfactory) .
