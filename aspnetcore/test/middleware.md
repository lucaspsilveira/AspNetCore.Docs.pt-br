...
O middleware pode ser testado isoladamente com <xref:Microsoft.AspNetCore.TestHost.TestServer>o. Ele permite que você:
...
* Crie e inicie um host que usa <xref:Microsoft.AspNetCore.TestHost.TestServer>o.
...
No exemplo a seguir, o middleware deve retornar um código de status 404 (*não encontrado*) quando o ponto de extremidade raiz é solicitado. Faça a execução do primeiro teste `Assert.NotEqual( ... );`com, que deve falhar:
...
<xref:Microsoft.AspNetCore.TestHost.TestServer.SendAsync%2A>permite a configuração direta de <xref:Microsoft.AspNetCore.Http.HttpContext> um objeto em vez de <xref:System.Net.Http.HttpClient> usar as abstrações. Use <xref:Microsoft.AspNetCore.TestHost.TestServer.SendAsync%2A> para manipular estruturas somente disponíveis no servidor, como [HttpContext. Items](xref:Microsoft.AspNetCore.Http.HttpContext.Items) ou [HttpContext. Features](xref:Microsoft.AspNetCore.Http.HttpContext.Features).
