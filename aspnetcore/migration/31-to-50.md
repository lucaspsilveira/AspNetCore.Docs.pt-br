...
Se você confiar em um arquivo [global. JSON](/dotnet/core/tools/global-json) para direcionar uma versão específica do SDK do .NET Core, `version` atualize a propriedade para a versão do SDK do .NET 5,0 que está instalada. Por exemplo: 
...
No arquivo de projeto, atualize cada `Microsoft.AspNetCore.*` e `Microsoft.Extensions.*` o atributo de `Version` referência de pacote para 5.0.0 ou posterior. Por exemplo: 
...
Para aplicativos que usam o Docker, use uma imagem base que inclua o tempo de execução ASP.NET Core 5,0. Por exemplo: 
