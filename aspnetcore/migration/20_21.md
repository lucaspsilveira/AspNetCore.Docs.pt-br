...
* O SDK do .NET Core 2.1.300 (incluído primeiro no Visual Studio 15,6) define a versão implícita `Microsoft.AspNetCore.App` do para 2.1.0 que causou conflitos com Entity Framework Core 2.1.1. A solução recomendada é atualizar o SDK do .NET Core para o 2.1.301 ou posterior. Para obter mais informações, consulte [pacotes que compartilham dependências com Microsoft. AspNetCore. app não podem fazer referência a versões de patch](https://github.com/aspnet/Universe/issues/1180).
* Todos os projetos que devem `Microsoft.AspNetCore.All` usar `Microsoft.AspNetCore.App` ou devem adicionar uma referência de pacote para o pacote no arquivo de projeto, mesmo que contenham uma referência de projeto `Microsoft.AspNetCore.All` a `Microsoft.AspNetCore.App`outro projeto usando o ou o.

  Exemplo:

  * `MyApp`tem uma referência de pacote `Microsoft.AspNetCore.App`para.
  * `MyApp.Tests`tem uma referência de projeto `MyApp.csproj`para.

  Adicione uma referência de pacote `Microsoft.AspNetCore.App` para `MyApp.Tests`o. Para obter mais informações, consulte o [teste de integração é difícil de configurar e pode interromper o serviço de estrutura compartilhada](https://github.com/dotnet/aspnetcore/issues/3156).
...
O código a seguir mostra as alterações no código gerado do modelo 2,1. Todas as alterações são um código recém-adicionado, exceto `UseBrowserLink` que foi removido:

[!code-csharp[](20_21/sample/Startup.cs?highlight=3,4,21-26,30,42,45,47)]

As alterações de código anteriores são detalhadas em:

* [Suporte do GDPR no ASP.NET Core](xref:security/gdpr) `CookiePolicyOptions` para `UseCookiePolicy`e.
...
* Quaisquer correções de bugs ou novos recursos para identidade estão incluídos no [metapacote Microsoft. AspNetCore. app](xref:fundamentals/metapackage-app). Você obtém automaticamente a identidade atualizada quando `Microsoft.AspNetCore.App` o é atualizado.
...
A identidade 2,1 expõe pontos de extremidade com `Identity` a área. Por exemplo, a tabela a seguir mostra exemplos de pontos de extremidade de identidade que mudam de 2,0 para 2,1:
...
Aplicativos que têm código usando Identity e substituem Identity a interface do usuário Identity 2,0 com a biblioteca 2,1 precisam Identity levar em `/Identity` conta as URLs têm um segmento precedido para os URIs. Uma maneira de lidar com os Identity novos pontos de extremidade é configurar redirecionamentos, por exemplo, de `/Account/Login` para `/Identity/Account/Login`.

### <a name="update-identity-to-version-21"></a>Atualizar Identity para a versão 2,1

As opções a seguir estão disponíveis para Identity atualizar para o 2,1.

* Use o Identity código da interface do usuário 2,0 sem alterações. O Identity uso do código de interface do usuário 2,0 é totalmente suportado. Essa é uma boa abordagem quando alterações significativas foram feitas no código gerado Identity .
...
### <a name="replace-identity-20-ui-with-the-identity-21-razor-class-library"></a>Substitua Identity a interface do usuário Identity 2,0 Razor pela biblioteca de classes 2,1

Esta seção descreve as etapas para substituir o código gerado pelo Identity modelo ASP.NET Core 2,0 pelo [ASP.NET Core Identity ](xref:security/authentication/identity) [ Razor biblioteca de classes](xref:razor-pages/ui-class). As etapas a seguir são para Razor um projeto de páginas, mas a abordagem para um projeto MVC é semelhante.
...
* Exclua Identity a classe `IdentityDbContext` derivada gerada scaffolder na pasta *areas/Identity/data/* .
* Excluir *áreas/Identity/IdentityHostingStartup.cs*.
* Atualize o * \_arquivo LoginPartial. cshtml* :
  * Mova *pages/\_LoginPartial. cshtml* para *páginas/Shared/\_LoginPartial. cshtml*.
...
## <a name="changes-to-razor-pages-projects-razor-files"></a>Alterações em Razor páginas arquivos Razor de projetos

### <a name="the-layout-file"></a>O arquivo de layout

* Mover *pages/\_layout. cshtml* para *páginas/Shared/\_layout. cshtml*
* Em *áreas/Identity/Pages/\_ViewStart. cshtml*, altere `Layout = "/Pages/_Layout.cshtml"` para `Layout = "/Pages/Shared/_Layout.cshtml"`.
* O * \_arquivo layout. cshtml* tem as seguintes alterações:
...
* *Pages/\_ValidationScriptsPartial. cshtml* move-se para *páginas/\_Shared/ValidationScriptsPartial. cshtml*.
...
## <a name="changes-to-mvc-projects-razor-files"></a>Alterações nos arquivos de Razor projetos do MVC
...
Os métodos <xref:Microsoft.AspNetCore.Mvc.ControllerBase> a seguir não processam mais o cabeçalho [Accept-Ranges](https://developer.mozilla.org/docs/Web/HTTP/Headers/Accept-Ranges) por padrão:
