...
* `<handlers>` Na entrada que adiciona o módulo ASP.NET Core (`name="aspNetCore"`), altere o `modules` valor do atributo de `AspNetCoreModule` para `AspNetCoreModuleV2`.
...
Se o aplicativo não chamar `CreateDefaultBuilder` `Program` e criar o host manualmente na classe, chame <xref:Microsoft.AspNetCore.Hosting.WebHostBuilderKestrelExtensions.UseKestrel*> **antes** de chamar `ConfigureKestrel`:
