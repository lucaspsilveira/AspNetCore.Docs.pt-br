...
Se sua solução depende de um arquivo [global. JSON](/dotnet/core/tools/global-json) para direcionar uma versão específica do SDK do .NET Core, atualize `version` sua propriedade para a versão 3,0 instalada em seu computador:
...
* Identity IU
  ...
  O suporte à [ Identity interface do usuário](xref:security/authentication/identity) pode ser adicionado referenciando o [MicrosoftIdentity. AspNetCore.. Pacote da interface do usuário](https://www.nuget.org/packages/Microsoft.AspNetCore.Identity.UI) .
...
* O &ndash; suporte de autenticação para fluxos de autenticação de terceiros está disponível como pacotes NuGet:
...
* O suporte à formatação e à `System.Net.HttpClient` &ndash; negociação de conteúdo para o pacote NuGet [Microsoft. AspNet. WebApi. Client](https://www.nuget.org/packages/Microsoft.AspNet.WebApi.Client/) fornece uma extensibilidade útil para `System.Net.HttpClient` o com APIs como `ReadAsAsync` e `PostJsonAsync`.

* Razoro suporte &ndash; à compilação em tempo de Razor execução para a compilação de Runtime de exibições e páginas agora faz parte de [Microsoft. AspNetCore. Mvc.Razor. RuntimeCompilation](https://www.nuget.org/packages/Microsoft.AspNetCore.Mvc.Razor.RuntimeCompilation).

* O `Newtonsoft.Json` suporte do mvc ( &ndash; JSON.net) para usar o `Newtonsoft.Json` MVC com o agora faz parte de [Microsoft. AspNetCore. Mvc. NewtonsoftJson](https://www.nuget.org/packages/Microsoft.AspNetCore.Mvc.NewtonsoftJson).

## <a name="startup-changes"></a>Alterações de inicialização

A imagem a seguir mostra as linhas excluídas e alteradas em Razor um aplicativo Web ASP.NET Core páginas 2,2:
...
A imagem a seguir mostra as linhas adicionadas e alteradas em Razor um aplicativo Web ASP.NET Core páginas 3,0:
...
RazorOs projetos de biblioteca de classes que fornecem componentes de interface do `AddRazorSupportForMvc` usuário para o MVC devem definir a propriedade no arquivo de projeto:
...
* `GetDeclaredTrailers`&ndash; Obtém o cabeçalho `Trailer` da solicitação que lista os marcadores a serem esperados após o corpo.
...
Se a e/s síncrona for necessária, ela poderá ser habilitada Configurando a `AllowSynchronousIO` opção no servidor que está sendo `ConfigureKestrel`usado (ao chamar, por exemplo, se estiver usando Kestrel). Observe que os servidores (Kestrel, HttpSa, TestServer, etc.) têm sua própria `AllowSynchronousIO` opção que não afetará outros servidores. A e/s síncrona pode ser habilitada para todos os servidores de acordo com a solicitação `IHttpBodyControlFeature.AllowSynchronousIO` usando a opção:
...
O serializador JSON padrão para ASP.NET Core é <xref:System.Text.Json>agora, que é novo no .net Core 3,0. Considere o `System.Text.Json` uso quando possível. Ele é de alto desempenho e não requer uma dependência de biblioteca adicional. No entanto `System.Text.Json` , como é novo, no momento, ele pode estar faltando recursos que seu aplicativo precisa. Para obter mais informações, consulte [como migrar de Newtonsoft. JSON para System. Text. JSON](/dotnet/standard/serialization/system-text-json-migrate-from-newtonsoft-how-to).

### <a name="use-newtonsoftjson-in-an-aspnet-core-30-signalr-project"></a>Usar Newtonsoft. JSON em um projeto ASP.NET Core SignalR 3,0
...
* No servidor, encadeamento uma `AddNewtonsoftJsonProtocol` chamada de método para a `AddSignalR` chamada de método `Startup.ConfigureServices`em:
...
Três novos métodos de extensão de nível superior relacionados aos cenários MVC `IServiceCollection` no estão disponíveis. Os modelos usam esses novos métodos em `AddMvc`vez de. No entanto, `AddMvc` continua se comportando como tem em versões anteriores.
...
O exemplo a seguir adiciona suporte Razor para páginas e suporte mínimo a controlador. O modelo de aplicativo Web usa este código:
...
* Se o aplicativo implementar `IRouter` ou herdar `Route`de, use [DynamicRouteValuesTransformer](https://github.com/dotnet/AspNetCore.Docs/issues/12997) como a substituição.
...
* Se o aplicativo usar recursos de autenticação/autorização, `AuthorizePage` como `[Authorize]`ou, coloque a chamada `UseAuthentication` para `UseAuthorization`e **after**: After `UseRouting` e `UseCors`, mas antes `UseEndpoints`de:
...
Se o aplicativo usar [manipuladores de autorização](xref:security/authorization/policies#authorization-handlers)personalizados, o roteamento de ponto de extremidade passará um tipo de recurso diferente para manipuladores do que o MVC. Os manipuladores que esperam que o recurso de contexto do manipulador de <xref:Microsoft.AspNetCore.Mvc.Filters.AuthorizationFilterContext> autorização sejam do tipo (o tipo de recurso [fornecido pelos filtros MVC](xref:security/authorization/policies#accessing-mvc-request-context-in-handlers)) precisarão ser atualizados para <xref:Microsoft.AspNetCore.Routing.RouteEndpoint> manipular recursos do tipo (o tipo de recurso fornecido aos manipuladores de autorização por roteamento de ponto de extremidade).

O MVC ainda `AuthorizationFilterContext` usa recursos, portanto, se o aplicativo usar filtros de autorização do MVC junto com a autorização de roteamento de ponto de extremidade, poderá ser necessário lidar com os dois tipos de recursos.

### SignalR

O mapeamento SignalR de hubs agora ocorre dentro `UseEndpoints`de.

Mapeie cada Hub com `MapHub`. Como nas versões anteriores, cada Hub é explicitamente listado.

No exemplo a seguir, o suporte para `ChatHub` SignalR o Hub é adicionado:
...
* Geração de link necessária ao `Async` especificar o sufixo. Por exemplo: 
...
* A geração de link não requer `Async` a especificação do sufixo. Por exemplo: 

    ```cshtml
    <a asp-controller="Products" asp-action="List">List</a>
    ```

Essa alteração não afeta os nomes especificados usando [`[ActionName]`](/dotnet/api/microsoft.aspnetcore.mvc.actionnameattribute) o atributo. O comportamento padrão pode ser desabilitado com o seguinte código `Startup.ConfigureServices`em:
...
Conforme explicado na documentação sobre [diferenças de versões anteriores do roteamento](xref:fundamentals/routing#differences-from-earlier-versions-of-routing), há algumas diferenças na geração de link ( `Url.Link` usando e APIs semelhantes, por exemplo). Elas incluem:
...
As Razor páginas de mapeamento agora ocorrem `UseEndpoints`dentro do.

Adicione `MapRazorPages` se o aplicativo usar Razor páginas. Como o roteamento de ponto de extremidade inclui suporte para muitas Razor estruturas, adicionar páginas agora é aceitar.

No método a `Startup.Configure` seguir, `MapRazorPages` o adiciona suporte Razor para páginas:
...
A alteração mais significativa de `WebHostBuilder` para `HostBuilder` está na [injeção de dependência (di)](xref:fundamentals/dependency-injection). Ao usar `HostBuilder`o, você só pode injetar o `Startup`seguinte Construtor into:
...
* Adicione uma referência de pacote a [Microsoft. AspNetCoreIdentity.. Interface do usuário](https://nuget.org/packages/Microsoft.AspNetCore.Identity.UI).
* Os aplicativos que não Razor usam páginas devem `MapRazorPages`chamar. Consulte [ Razor as páginas](#razor-pages) deste documento.
...
Se você estiver usando [recursos do Newtonsoft. JSON que não têm suporte em System. Text. JSON](/dotnet/standard/serialization/system-text-json-migrate-from-newtonsoft-how-to), poderá voltar para `Newtonsoft.Json`. Consulte [usar Newtonsoft. JSON em um projeto ASP.NET Core SignalR 3,0](#use-newtonsoftjson-in-an-aspnet-core-30-signalr-project) anteriormente neste artigo.
...
Para obter mais informações Razor sobre a compilação de <xref:mvc/views/view-compilation>arquivos, consulte.
...
Para obter mais informações sobre como usar ASP.NET Core APIs em uma biblioteca de <xref:fundamentals/target-aspnetcore>classes, consulte.
