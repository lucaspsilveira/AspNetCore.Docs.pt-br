...
* Trate qualquer método .NET exposto ao JavaScript como você faria com um ponto de extremidade público para o aplicativo.
  ...
  * Leve em consideração que os métodos estáticos e de instância podem ser expostos a clientes JavaScript. Evite compartilhar o estado entre as sessões, a menos que o design chame o estado de compartilhamento com as restrições apropriadas.
    * Para métodos de instância expostos `DotNetReference` por meio de objetos criados originalmente por injeção de dependência (di), os objetos devem ser registrados como objetos com escopo. Isso se aplica a qualquer serviço de DI Blazor que o aplicativo do servidor usa.
...
* Evite implementar a expedição personalizada de invocações do .NET sobre a implementação de expedição da estrutura. Expor métodos .NET ao navegador é um cenário avançado, não recomendado para desenvolvimento geral Blazor .
...
Se um retorno de chamada de evento invocar uma operação de execução longa de forma assíncrona, como a busca de dados de um serviço externo ou banco de dado, considere o uso de uma proteção. A proteção pode impedir que o usuário enfileirar várias operações enquanto a operação está em andamento com comentários visuais. O código de componente a `isLoading` seguir `true` define `GetForecastAsync` enquanto obtém dados do servidor. Enquanto `isLoading` estiver `true`, o botão será desabilitado na interface do usuário:
...
O padrão de proteção demonstrado no exemplo anterior funcionará se a operação em segundo plano for `async` - `await` executada de forma assíncrona com o padrão.
...
As diretrizes para proteger aplicativos ASP.NET Core se aplicam Blazor a aplicativos de servidor e são abordadas nas seguintes seções:
...
As interações de interoperabilidade do JS entre o cliente e o servidor são <xref:Microsoft.Extensions.Logging.ILogger> registradas nos logs do servidor com instâncias. Blazorevita informações confidenciais de log, como eventos reais ou entradas e saídas de interoperabilidade do JS.
...
BlazorO servidor SignalR usa para a comunicação entre o cliente e o servidor. BlazorO servidor normalmente usa o transporte SignalR que negocia, que normalmente é WebSockets.
...
* Verifique se as configurações de CORS são apropriadas ao habilitar o CORS ou Blazor desabilitar explicitamente o CORS para aplicativos.
* Teste para garantir que os limites do lado do servidor para Blazor o aplicativo forneçam uma experiência de usuário aceitável sem níveis inaceitáveis de risco.
